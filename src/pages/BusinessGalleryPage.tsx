...
const translations = {
    en: {
        ...
        "Grid work": "Grid work",
        "Play": "Play", // Or "View", "Watch" depending on context
    },
    ar: {
        "Clear all filters": "مسح جميع الفلاتر",
        "Search from here": "البحث من هنا",
        "Search...": "بحث...",
        "Categories": "التصنيفات",
        "Business gallery": "معرض الأعمال",
        "Grid work": "عرض شبكي",
        "Play": "تشغيل", // or "عرض", "مشاهدة"
    },
};
...
const BusinessGalleryPage: React.FC = () => {
    ...
    const galleryItems: GalleryItem[] = Array(15).fill(null).map((_, index) => ({
        title: 'Design magazine',
    ...
    return (
        <div dir={language === 'ar' ? 'rtl' : 'ltr'} className="bg-gray-100 mt-24 min-h-screen flex">
            {/* Left Sidebar (Filters) */}
            <aside className="bg-white w-64 p-4 md:p-6 border-r border-gray-200"> {/* Responsive padding */}
                ...
                <div className="mb-4 md:mb-6"> {/* Responsive margin */}
                    <label htmlFor="search" className="block text-sm font-semibold text-gray-700 mb-2">{translate("Search from here", language)}</label>
                    <input
                        type="text"
                        id="search"
                ...
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">{translate("Categories", language)}</label>
                    <div className="border border-gray-300 rounded p-2">
                        {categories.map((category, index) => (
                            <div key={index} className="flex items-center mb-2">
                                <input type="checkbox" id={`category-${index}`} className="form-checkbox h-4 w-4 text-orange-500 rounded border-gray-300 focus:ring-orange-500" />
            ...
            {/* Main Content (Gallery) */}
            <main className="flex-1 p-4 md:p-8"> {/* Responsive padding */}
                <div className="flex justify-between items-center mb-4 md:mb-6"> {/* Responsive margin */}
                    <div>
                        <h2 className="text-xl font-semibold text-gray-800">{translate("Business gallery", language)}</h2>
                    </div>
                    <div>
                        <button className="bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline text-sm md:text-base"> {/* Responsive text size */}
                            {translate("Grid work", language)}
                        </button>
                    </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6"> {/* Responsive grid and gap */}
                    {galleryItems.map((item, index) => (
                        <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden">
                            <img className="w-full h-32 object-cover" src={item.imageUrl} alt={item.title} />
                            <div className="p-4">
                                ...
                                <div className="flex items-center justify-between text-gray-600 text-xs">
                                    <div className="flex items-center space-x-2">
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M14 10h-2v2h2v-2zM3 21v-2a4 4 0 014-4h10a4 4 0 014 4v2M3 7v-2a4 4 0 014-4h10a4 4 0 014 4v2h-1.333M3 7h18m-13 4v4h-2v-4m-6-4h.01M19 11h.01M21 7a2 2 0 01-2 2V5a2 2 0 112 2zM4 7a2 2 0 01-2 2V5a2 2 0 112 2z"></path></svg>
                                        <span>{translate("Play", language)}</span> {/* Translated "Play" */}
                                    </div>
                                    <div className="flex items-center space-x-4">
                                        <div className="flex items-center space-x-1">
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
